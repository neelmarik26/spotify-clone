...
// FUNCTION TO GET ALBUM ART FROM MP3 FILE
...
async function main() {
  ...
  for (let index = 0; index < song_list.length; index++) {
    ...
    getAlbumArtWithLibrary(songUrl, function(coverUrl) {
      ...
      img.onerror = function() {
        // If the extracted image fails to load, use default
        this.src = 'covers/default.jpg';
      };
    });
  }
 
  // ad an event listioner too all songs LIST
  ...
  document.getElementById('previes').addEventListener('click', () => {
    if (current_song.src == '') {
      ...
      console.log(song_list[current_index - 1]);
      if (current_index <= 0) {
        play_music(song_list[0]);
      } else {
